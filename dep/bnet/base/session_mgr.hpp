...
namespace bnet::base {
	...
	public:
		...
		inline session_ptr get_by_hash(size_t hash) {
			std::unique_lock<std::shared_mutex> guard(this->mutex_);
			if (sessions_.size() <= 0) {
				return session_ptr();
			}
			
			auto offset = hash % sessions_.size();
			auto itr = std::next(std::begin(sessions_), offset);
			return (itr == this->sessions_.end() ? session_ptr() : itr->second);
		}

		inline session_ptr get_by_key(const key_type& key) {
			return find(key);
		}
	protected:
