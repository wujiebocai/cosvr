...
namespace bnet::base {
	...
	public:
		...
		inline bool add(std::string_view host, std::string_view port, const CompletionToken& token = asio::detached, session_ptr_type* ptr = nullptr) {
			if (!is_started()) {
				return false;
			}
			...
			auto session_ptr = this->make_session();
			if (ptr) {
				*ptr = session_ptr;
			}
