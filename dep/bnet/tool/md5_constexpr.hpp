...
constexpr uint32_t MD5Hash64Constexpr(const char *string) {
  return MD5CE::Hash64(string, MD5CE::StringLength(string));
}
constexpr uint32_t MD5Hash64Constexpr(const char *string, uint32_t length) {
  return MD5CE::Hash64(string, length);
}
