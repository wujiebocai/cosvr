...
class executor_wrapper : public async_simple::Executor {
private:
    ExecutorImpl executor_;

public:
    using context_t = std::remove_cvref_t<decltype(executor_.context())>;
    executor_wrapper(ExecutorImpl executor) : executor_(executor) {}
...
public:
    ...
    explicit io_context_pool(std::size_t pool_size) : next_io_context_(0) {
        ...
        for (std::size_t i = 0; i < pool_size; ++i) {
            ...
            auto executor = std::make_unique<executor_wrapper<>>(
            io_context->get_executor());
            executors.push_back(std::move(executor));
            work_.emplace_back(io_context->get_executor());
        }
    }
...
private:
    ...
    std::vector<std::unique_ptr<executor_wrapper<>>> executors;
...
public:
    multithread_context_pool(size_t thd_num = std::thread::hardware_concurrency())
        : work_(std::make_unique<asio::executor_work_guard<asio::io_context::executor_type>>(ioc_.get_executor()))
